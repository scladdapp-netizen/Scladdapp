...
const PROFILE_ITEMS = [
  { id: "info",      label: "Class Info",      icon: Icons.Home },
  { id: "subjects",  label: "Class Subjects",  icon: Icons.Subject },
  { id: "resources", label: "Class Resources", icon: Icons.Class },
];
...
const ClassDashboardSidebar = ({ sessionData = [] }) => {
  const { schoolId, classId } = useParams();
  const navigate = useNavigate();
  const location = useLocation();

  const activeSubseasion = location.state?.subseasion || null;
  ...
  const isSessionNavActive = (subId, navId) =>
    location.pathname === `${base}/${navId}` && location.state?.subseasion === subId;
  ...
  const handleSessionNavClick = (subId, navId) =>
    navigate(`${base}/${navId}`, { state: { subseasion: subId } });
        ...
        <div className="session-folder">
          <div className="sidebar-item" onClick={() => handleProfileClick("info")}>
            <div className="session-content">
              ...
              <div className="session-text">
                <p className="session-name">Class Profile</p>
              </div>
            </div>
          </div>
          <div className="session-navigation">
            {PROFILE_ITEMS.map((item) => (
              ...
              >
                <item.icon size={18} color={isProfileActive(item.id) ? "#4f46e5" : "#6b7280"} />
                <p>{item.label}</p>
              </div>
            ))}
          </div>
...
export default ClassDashboardSidebar;
